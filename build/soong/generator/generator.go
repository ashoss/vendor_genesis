...
func init() {
	android.RegisterModuleType("genesis_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = genesisExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = genesisExpandVariables(ctx, dep_file)
	...
	cmd := genesisExpandVariables(ctx, String(g.properties.Cmd))
