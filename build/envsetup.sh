...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^genesis_") ; then
        GENESIS_BUILD=$(echo -n $1 | sed -e 's/^genesis_//g')
    else
        GENESIS_BUILD=
    fi
    export GENESIS_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/genesis/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Genesis model name
            if [ -z "$variant" ]; then
                variant="user"
            fi

            lunch genesis_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/GenesisOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.genesis.device | grep -q "$GENESIS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $GENESIS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/GenesisOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.genesis.device | grep -q "$GENESIS_BUILD");
    then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                genesis|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function dopush()
{
    ...
    if (adb shell getprop ro.genesis.device | grep -q "$GENESIS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $GENESIS_FIXUP_COMMON_OUT ]; then
